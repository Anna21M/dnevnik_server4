// код: изменить оценку для 2 четврети
...
async function getGradeMemory() {
  const res = await fetch("/quarter_2_get_memory", { 
      method: "POST"    
    })          
...
but_update.addEventListener('click', async ()=>{
    ...
    const res = await fetch("/quarter_2_db_update", {       
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id:id_grade, grade:   // id оценки
          pole_grade.value      // оценка
        }) 
      })          
      const result = await res.json()     
