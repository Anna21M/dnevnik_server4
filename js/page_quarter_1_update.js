// Код: изменить оченку для 1 четверти
...
async function getGradeMemory() {
  const res = await fetch("/quarter_1_get_memory", { 
      method: "POST"    
    })          
...
// Выполняем выше описанные запросы
...
but_update.addEventListener('click', async ()=>{
    ...
    const res = await fetch("/quarter_1_db_update", {       
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id:id_grade,    // id оценки
