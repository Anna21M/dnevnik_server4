// код: список оценок для 4 черверти
...
list.onclick = async function(event) {
    ...
    switch(tag_class) {
        ...
        case "but_update":
            ...
            const res = await fetch("/quarter_4_put_memory", { 
                method: "POST",
              ...
              if (result.goodMemory) {
                ...
                document.location.href = "/page_quarter_4_update" 
              }
        break
        // Нажата кнопка "Удалить"
        case "but_delete":
            const resDel = await fetch("/quarter_4_db_delete", { 
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({id: tag_id}) 
              })          
...
// Запрос у сервера списка всех оценок (4 четверть)
async function getAllGrades() {
    const res = await fetch("/quarter_4_list", { 
        method: "POST"    
      })          
