...
list.onclick = async function(event) {
    ...
    switch(tag_class) {
        ...
        case "but_update":
            ...
            const res = await fetch("/quarter_1_put_memory", { 
                method: "POST",
              ...
              if (result.goodMemory) {
                ...
                document.location.href = "/page_quarter_1_update" 
              }
        break
        // Нажата кнопка "Удалить"
        case "but_delete":
            const resDel = await fetch("/quarter_1_db_delete", { 
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({id: tag_id}) 
              })          
              ...
              if (resultDel.goodDelete) {
                document.location.href = "/page_quarter_1" 
              } 
    }    
  }


// Запрос у сервера списка всех оценок (1 четверть)
async function getAllGrades() {
    const res = await fetch("/quarter_1_list", { 
        method: "POST"    
      })          
